...
public:
	bool isBusy();
	bool doInvoker(const std::function<bool()>& callImpl, const InvokerNotify& cb, bool blocking = false);
	bool doInvoker(const InvokerCall& callImpl, const CallParamIn& paramIn, const InvokerNotify& cb, bool blocking = false);

	//放到对列中异步执行
	bool doInvokerInQueue();

protected:

	AsyncEventLoopInvoker();
